...
async function discoverSkFiles() {
  try {
    ...
    if (found.length) {
      ...
      if (added.length)
        console.log(
          `[pyq] discovered ${added.length} new machine-readable paper(s): ${added.join(", ")}`,
        );
      return found;
    }
  } catch (e) {
    console.warn(
      "[pyq] paper discovery unavailable (" + e.message + ") — using the pinned baseline list.",
    );
  }
  return SK_FILES;
}
...
function cleanHTML(raw) {
  ...
  t = t.replace(
    /<\/?(p|b|strong|i|em|u|s|span|div|ul|ol|li|table|tbody|thead|tr|th|h[1-6]|a|center|font|small|big|blockquote|pre|code|section|figure|figcaption)(\s[^>]*)?\/?>/gi,
    "",
  );
  t = t
    .replace(/&nbsp;/gi, " ")
...
function cleanText(s) {
  return collectImages(cleanHTML(s));
}
...
function transformRow(row) {
  ...
  if (qtype === "integer") {
    ...
    try {
      answer = String(JSON.parse(String(row.correct_option || "[]"))[0] || "").toLowerCase();
    } catch {
      return null;
    }
...
function parsePaperId(id) {
  ...
  if (m) {
    ...
    return { year: +m[1], month, label: `${m[2]} ${month.slice(0, 3)} · ${pretty(m[4])} Shift` };
  }
...
const PHY_T =
  /physic|mechanic|thermo|optic|electro|magnet|current|wave|kinemat|gravit|semicond|units|dimension|modern|nuclei|atom|dual|communication|alternating|capacit|oscillat|motion|power|energy|momentum|elastic|fluid|kinetic/i;
const CHE_T =
  /chem|organic|inorganic|periodic|bond|acid|base|salt|solution|equilibri|electrochem|metallurg|polymer|biomolec|alcohol|alde|amine|halo|hydrocarbon|coordination|block|table|mole|atomic structure|surface|environment/i;
const MAT_T =
  /math|algebra|calculus|integr|differenti|matrix|matrices|determinant|vector|probability|statistic|trigono|geometr|parabola|ellipse|hyperbola|circle|line|series|sequence|binomial|permutation|complex|function|limit|continuity|set|relation|reasoning/i;
...
function skTransform(rows, paperId) {
  ...
  rows.forEach((row, i) => {
    ...
    out.push({
      paperId,
      no: 0,
      subject: skSubject(row, i, rows.length),
      chapter: pretty(String(row.topic || "")),
      topic: pretty(String(row.topic || "")),
      type: isInt ? "integer" : "mcq",
      text: cleanText(text),
      options: options.map((o) => ({ ...o, text: cleanText(o.text) })),
      answer,
      ...(accept ? { accept } : {}),
...
function skPaperId(name) {
  ...
  return {
    id: `jee-main-${m[1]}-online-${m[2]}-${month.toLowerCase()}-${m[4] === "1" ? "morning" : "evening"}-shift`,
    year: +m[1],
    month,
...
async function fromLatestJson(files) {
  ...
  for (const name of files || SK_FILES) {
    ...
    try {
      ...
      if (qs.length >= 30) {
        out.rowsByPaper.set(meta.id, qs);
        out.metas.push(meta);
      }
    } catch {
      /* skip one bad file, keep the rest */
    }
  }
  return out;
}
...
async function fromLocalJson() {
  ...
  let names = [];
  try {
    names = (await readdir(LOCAL_PYQ_DIR)).filter((n) => n.endsWith(".json")).sort();
  } catch {
    return out;
  } // dir absent → nothing to add (safe)
...
async function fromRowsApi() {
  const first = await fetch(`${ROWS_API}&offset=0&length=100`, {
    signal: AbortSignal.timeout(30_000),
  });
  ...
  for (let i = 0; i < offsets.length; i += 5) {
    ...
    const results = await Promise.allSettled(
      batch.map(async (o) => {
        const r = await fetch(`${ROWS_API}&offset=${o}&length=100`, {
          signal: AbortSignal.timeout(30_000),
        });
        if (!r.ok) throw new Error(String(r.status));
...
async function main() {
  ...
  let rows = null;
  try {
    rows = await fromParquet();
    console.log(`[pyq] parquet snapshot: ${rows.length} rows`);
  } catch (e) {
    console.log("[pyq] parquet failed (" + e.message + "), trying rows API …");
    try {
      rows = await fromRowsApi();
      console.log(`[pyq] rows API: ${rows.length} rows`);
    } catch (e2) {
      console.warn("[pyq] snapshot dataset unavailable (" + e2.message + ")");
    }
  }
  ...
  if (latest.metas.length)
    console.log(
      `[pyq] latest-session source: ${latest.metas.length} paper(s) (${latest.metas.map((m) => m.year).join(", ")})`,
    );
  // Source 3: user-transcribed JEE 2026 papers (offline, committed).
  const local = await fromLocalJson();
  if (local.metas.length)
    console.log(`[pyq] user-transcribed source: ${local.metas.length} paper(s)`);

  if (!rows && !latest.metas.length && !local.metas.length) {
    console.warn(
      "[pyq] WARNING: no source reachable. Site still builds; the PYQ tab will use the API fallback until the next build.",
    );
  ...
  await writeFile(
    join(OUT, "index.json"),
    JSON.stringify({ v: 1, builtAt: Date.now(), papers: index }),
  );
  ...
  if (IMG_TASKS.size)
    console.log(`[pyq] figures: ${gotImgs}/${IMG_TASKS.size} downloaded into public/pyq/img/`);
  const totalQ = index.reduce((s, p) => s + p.total, 0);
  const years = [...new Set(index.map((p) => p.year))].sort((a, b) => b - a);
  console.log(
    `[pyq] baked ${index.length} papers (${years[years.length - 1]}–${years[0]}), ${totalQ} questions → public/pyq/`,
  );
}

main().catch((e) => {
  console.warn("[pyq] non-fatal:", e.message);
});
